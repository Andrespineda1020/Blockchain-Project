...
for (let i = 0; i < myWallet.collectibles.length; i++) {
  chainList[myWallet.collectibles[i]] = require('../jsons/' + myWallet.collectibles[i].name + '.json');
}
...
app.post('/join', (req, res) => {
  try {
    ...
    fs.writeFile('./src/jsons/' + blockName + '.json', JSON.stringify(newBlockchain), 'utf8', (err) => {
      if (err) console.log(err)
...
requestBlock = (blockName, userAddress, genesis) => {
  ...
  try {
    axios.post(address + genesis + '/request', body)
      .then((res) => {
        ...
        for (let i = genesis + 1; i != genesis; i = blockchain.wallets[i].next) {
          for (let j = 0; j < blockchain.wallets[i].collectibles.length; i++) {
...
app.get('/request', (req, res) => {
  try {
    ...
    fs.writeFile('./src/jsons/' + blockname + '.json', JSON.stringify(chainList[blockname]), 'utf8', (err) => {
      if (err) console.log(err)
      else console.log("Local Blockchain Updated!");
    });
