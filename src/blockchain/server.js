...
for (let i = 0; i < myWallet.collectibles.length; i++) {
    chainList[myWallet.collectibles[i]] = require("../jsons/" +
        myWallet.collectibles[i] +
        ".json");
}
...
app.post("/create", (req, res) => {
    try {
        ...
        let ranking =
            chainList[name].wallets.length -
            chainList[name].wallets[block.address1] +
            chainList[name].wallets[block.address2];
...
validateTransaction = (transaction, name) => {
    ...
    if (Block.mineBlock(transaction)) {
        ...
        transaction.currentRank =
            transaction.currentRank +
            (chainList[name].wallets.length - myWallet.ranking);
